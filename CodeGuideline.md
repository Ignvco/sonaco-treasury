...
## Treasury quality modules (September 2026)
- `src/import-engine/`: shared browser import detection, normalization, validation and preview contracts.
- `src/services/importService.ts`: worker lifecycle and authenticated atomic database import.
- `src/pages/importations/ImportPreview.tsx`: worksheet mapping and validation review.
- `tests/`: regression tests for Excel, financial math, persistence, permissions and browser flows.
- `vendor/xlsx-0.20.3.tgz`: pinned upstream SheetJS distribution.
- `supabase/migrations/20260914000000000_import_integrity.sql`: atomic import and role protection.

- `src/layouts/Sidebar.tsx`: grouped navigation and accessible mobile drawer.
- `public/locales/es.json`: Spanish fallback for the treasury interface.

- `src/financial-engine/currency.ts`: explicit CLP conversion with validated rates.
- `playwright.config.ts` and `tests/browser/`: reproducible browser tests using synthetic data and intercepted API responses.

- `README.md`, `ACTUALIZACION.md` and `AUDITORIA.md`: installation, deployment requirements, verified findings and known limits.
- `docs/screenshots/`: browser captures with synthetic data, regenerated by browser tests.

## BASE treasury review (v6)
- `src/financial-engine/base-treasury.ts`: BASE signed balances, native-currency filters, forecast events and explicit anti-duplication links.
- `src/services/baseTreasuryService.ts`: current BASE traces, native values and forecast links.
- `src/pages/dashboard/ForecastLinks.tsx`: review manual/ERP collection equivalences.
- `src/components/treasury/SourceBreakdown.tsx`: financial contribution lists and original BASE cell values.
- `src/hooks/use-saved-filters.ts`: user-scoped view preferences on this browser.
- `supabase/migrations/20260914030000000_base_treasury_v6.sql`: daily comparison, optimistic revision check, in-place updates and retained history.
- `tests/base-treasury.test.ts`: source balance, forecast and link regression cases.
- `PATCH_5_MEJORAS.md`: patch application and the outstanding verification gate.

- `tests/verify-original-base.mjs`: local read-only verification of the original workbook; never bundles the client file.
- `ACTUALIZAR_5_MEJORAS.sql`: transaction-wrapped v6 migration for the SQL Editor.
