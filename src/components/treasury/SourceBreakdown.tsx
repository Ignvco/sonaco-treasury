...
export function SourceBreakdown({title,rows,onClose}:{title:string;rows:(TreasuryRow|ForecastEvent)[];onClose:()=>void}) {
 ...
 const open=async(row:TreasuryRow)=>{
   setDetail(null);setError("");if(!row.recordId){setDetail({title:"Ingreso en plataforma",raw:{Descripción:row.description,Fecha:row.date,Monto:row.amount,Moneda:row.currency}});return;}
   setBusy(true);try{const d=await baseTreasuryService.trace(row.recordId);setDetail({title:row.fileName+" · "+d.source_sheet+" · fila "+d.source_row,raw:d.raw_json});}
   catch(e){setError(e instanceof Error?e.message:"No se pudo cargar el origen.");}finally{setBusy(false);}
 };
 return <Dialog open onOpenChange={open=>{if(!open)onClose();}}><DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-5xl">
  ...
  <p className="text-right text-sm font-semibold tabular-nums">Suma de los valores mostrados: {baseNumber(total(rows.map(amount)))}</p>
