...
import { baseTreasury, isTreasuryDate, type TreasuryRow, type ForecastEvent } from "@/financial-engine/base-treasury";
...
export default function Dashboard() {
 ...
 const cutoff=isTreasuryDate(filters.cutoff)?filters.cutoff:data?.cutoff||"2026-01-01";
 ...
 return <div className="t-fade-in min-w-0 space-y-6">
  ...
  {data.warning&&<p role="alert" className="rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900">{data.warning}</p>}
  ...
  <div className="grid gap-5 xl:grid-cols-[minmax(0,2.3fr)_minmax(280px,1fr)]">
   ...
   <aside className="space-y-4 rounded-2xl border bg-[#f5f7fc] p-5"><h2 className="font-semibold">Por revisar</h2>
    ...
    {model.omittedRows.length>0&&<button className="w-full rounded-xl border bg-white p-4 text-left" onClick={()=>open("Registros fuera de la última BASE",model.omittedRows)}><p className="text-sm font-semibold">{model.omittedRows.length} registros de cargas anteriores</p><p className="mt-2 text-xs text-muted-foreground">No están en la última BASE. Se conservan para revisión y quedan fuera de la caja y la proyección activa.</p></button>}
