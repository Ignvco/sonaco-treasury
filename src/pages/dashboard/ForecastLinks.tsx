...
export function ForecastLinks({rows,links,onClose,onRefresh}:{rows:TreasuryRow[];links:ForecastLink[];onClose:()=>void;onRefresh:()=>void}) {
 ...
 const targets=rows.filter(r=>["invoice","investment"].includes(r.kind)&&!["pagado","rescatada"].includes(r.status)&&r.currency===p?.currency&&Math.abs(r.amount+r.interest-(p?.amount??0))<0.005&&!links.some(l=>l.target_id===r.id));
