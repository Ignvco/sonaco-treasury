...
returns text language sql immutable as $$
 ...
 case when p_kind='invoice' then upper(regexp_replace(coalesce(n->>'rut',n->>'customer',''),'[^a-zA-Z0-9]','','g')) else n->>'bank' end,
 n->>'document',
 case when p_kind='invoice' then null else n->>'voucher' end,
 case when p_kind='invoice' then null when p_kind='investment' then n->>'startDate' when p_kind='projection' then null else n->>'date' end,
 case when p_kind='invoice' then null else n->>'description' end)::text)
$$;
...
declare r jsonb; n jsonb; k text; item jsonb; candidate jsonb; candidates jsonb;
 result jsonb:='[]'; used_ids text[]:='{}'; incoming_count int; decision text; state text;
 groups jsonb; exact_rows jsonb; incoming_counts jsonb;
begin
 ...
 select coalesce(jsonb_object_agg(k,items),'{}') into groups from (
  select public.base_business_key(value->>'entity_type',value->'normalized_json') k,jsonb_agg(value) items from jsonb_array_elements(candidates) group by 1) g;
 select coalesce(jsonb_object_agg(k,items),'{}') into exact_rows from (
  select value->'normalized_json'->>'sourceId' k,jsonb_agg(value) items from jsonb_array_elements(candidates)
  where value->'normalized_json'->>'sourceId' is not null group by 1) g;
 select coalesce(jsonb_object_agg(k,n),'{}') into incoming_counts from (
  select public.base_business_key(value->>'entityType',value->'normalized') k,count(*) n from jsonb_array_elements(p_records) group by 1) g;
 for r in select value from jsonb_array_elements(p_records) loop
  ...
  if upper(trim(r->>'sheet')) is distinct from 'BASE' or coalesce(n->>'sourceProfile','') not like 'BASE-ONLY-%'
     or r->>'entityType' not in ('cash_flow','invoice','investment','projection')
     or r->>'status' not in ('VALID','WARNING') then
    decision:='invalid';
  else
    k:=public.base_business_key(r->>'entityType',n);
    -- Match exact content first, retaining distinct equal accounting postings.
    select value into candidate from jsonb_array_elements(coalesce(exact_rows->(n->>'sourceId'),'[]'))
    where value->>'entity_type'=r->>'entityType'
      and not (value->>'entity_id'=any(used_ids))
      and value->'normalized_json'->>'sourceId'=n->>'sourceId' limit 1;
    ...
    if candidate is not null then
      used_ids:=array_append(used_ids,candidate->>'entity_id');
  ...
  result:=result||jsonb_build_array(jsonb_build_object('row',r->'row','change',decision,
    'entityId',candidate->'entity_id','before',candidate->'current_normalized',
    'after',n,'reason',case when decision='conflict' then 'Varias partidas podrían corresponder a esta fila. Se conserva lo guardado; revisa el comprobante.' else null end));
 end loop;
 -- Revision includes live values and statuses, so stale previews cannot overwrite concurrent edits.
 select md5(coalesce(string_agg(value::text,'' order by value->>'entity_id'),'')||p_records::text)
 into state from jsonb_array_elements(candidates);
 return jsonb_build_object('revision',state,'rows',result);
end; $$;
